...
export default function CalendarIntelligencePage() {
  ...
  function ScoreDrawerContent({ row }: { row: CalendarConferenceRow }) {
    ...
    const components: { key: string; detailsKey: keyof NonNullable<CalendarConferenceRow['componentDetails']>; score: number | null; weight: number; unavailable?: string }[] = [
      { key: 'Audience Fit', detailsKey: 'audienceFit', score: cs?.audienceFit ?? null, weight: W.audienceFit },
      { key: 'Target Opportunity', detailsKey: 'targetOpportunity', score: cs?.targetOpportunity ?? null, weight: W.targetOpportunity, unavailable: te == null ? 'Prospect company type not configured.' : undefined },
      { key: 'Commercial Potential', detailsKey: 'commercialPotential', score: cs?.commercialPotential ?? null, weight: W.commercialPotential, unavailable: cp == null ? 'Commercial inputs unavailable.' : undefined },
      { key: 'Cost Justification', detailsKey: 'costJustification', score: cs?.costJustification ?? null, weight: W.costJustification, unavailable: bud == null ? 'No budget data available. Add budget in conference settings — this would add up to 18 points to your score.' : undefined },
      { key: 'Strategic Value', detailsKey: 'strategicValue', score: cs?.strategicValue ?? null, weight: W.strategicValue, unavailable: te == null ? 'Prospect company type not configured. This would add up to 10 points to your score.' : undefined },
    ];
              ...
              <div key={c.key} className="border rounded-lg p-3">
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5 flex-shrink-0">
                    <span className="text-xs text-gray-500">{c.score == null ? '—' : Math.round(c.score)}/100 · {c.weight}%{c.score == null ? ' — not scored' : ''}</span>
                    {details.length > 0 && (
                      <i
        ...
        {row.strategyScores && row.strategyScores.length > 0 && row.recommendedStrategy && (
          <div className="mt-5 pt-5 border-t">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-bold uppercase tracking-wide text-gray-400">Strategy fit</p>
