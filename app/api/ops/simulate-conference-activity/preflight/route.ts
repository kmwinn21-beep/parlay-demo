...
async function getTenantClient(accountId: string) {
  await dbReady;
...
// GET /api/ops/simulate-conference-activity/preflight?accountId=...&conferenceId=...
export async function GET(request: NextRequest) {
  ...
  const client = await getTenantClient(accountId);
  if (!client) {
...
// POST /api/ops/simulate-conference-activity/preflight
export async function POST(request: NextRequest) {
  ...
  const { accountId, conferenceId, fixes } = body;
