...
export async function GET(
  ...
  try {
    ...
    const repVisualRows = repAttribution.map((r) => {
      const salesActivities = Number(r.meetings_held ?? 0) + Number(r.touchpoints ?? 0);
      const pipe = Number(r.pipeline_influence_attributed ?? 0);
      return {
        rep_id: String(r.rep),
        rep_name: String(r.rep),
        sales_activities: salesActivities,
        pipeline_influence: pipe,
        meetings_held: Number(r.meetings_held ?? 0),
        touchpoints_logged: Number(r.touchpoints ?? 0),
        companies_engaged: Number(r.unique_companies_met ?? 0),
        followup_completion_rate: null,
      };
    });
    const avgActivity = repVisualRows.length ? repVisualRows.reduce((a,r)=>a+r.sales_activities,0)/repVisualRows.length : null;
    const avgPipeline = repVisualRows.length ? repVisualRows.reduce((a,r)=>a+r.pipeline_influence,0)/repVisualRows.length : null;
    const teamPPSA = salesActivities > 0 ? totalPipelineInfluence / salesActivities : null;
    ...
    return NextResponse.json({
      ...
      sales_visuals: {
        rep_execution_quadrant: {
          reps: repVisualRows.map((r) => ({ ...r, quadrant: (avgActivity != null && avgPipeline != null)
            ? (r.sales_activities >= avgActivity && r.pipeline_influence >= avgPipeline ? 'top_performer'
              : r.sales_activities >= avgActivity ? 'busy_low_yield'
              : r.pipeline_influence >= avgPipeline ? 'strategic_under_leveraged'
              : 'low_impact')
            : null })),
          activity_benchmark: avgActivity,
          pipeline_benchmark: avgPipeline,
        },
        rep_score_breakdown: {
          reps: repVisualRows.map((r) => ({
            rep_id: r.rep_id,
            rep_name: r.rep_name,
            meeting_execution_score: r.meetings_held > 0 ? Math.min(100, Math.round((r.meetings_held / Math.max(r.meetings_held, 1)) * 100)) : null,
            followup_execution_score: null,
            pipeline_influence_execution_score: avgPipeline && avgPipeline > 0 ? Math.min(100, Math.round((r.pipeline_influence / avgPipeline) * 100)) : null,
            target_account_execution_score: null,
            rep_productivity_score: avgActivity && avgActivity > 0 ? Math.min(100, Math.round((r.sales_activities / avgActivity) * 100)) : null,
            total_sales_effectiveness_score: null,
          })),
        },
        pipeline_contribution_by_rep: {
          reps: repVisualRows.map((r) => ({ ...r, contribution_percent: totalPipelineInfluence > 0 ? (r.pipeline_influence / totalPipelineInfluence) * 100 : 0 })),
          total_pipeline_influence: totalPipelineInfluence,
          top_rep_contribution_percent: totalPipelineInfluence > 0 && repVisualRows.length ? Math.max(...repVisualRows.map(r => r.pipeline_influence)) / totalPipelineInfluence * 100 : null,
          top_three_contribution_percent: totalPipelineInfluence > 0 && repVisualRows.length ? repVisualRows.map(r=>r.pipeline_influence).sort((a,b)=>b-a).slice(0,3).reduce((a,b)=>a+b,0) / totalPipelineInfluence * 100 : null,
        },
        activity_mix_by_rep: {
          reps: repVisualRows.map((r) => ({ ...r, followups_created: 0 })),
        },
        followup_funnel: {
          sales_activities: salesActivities,
          meetings_held: salesMeetingsHeld,
          companies_with_followup_created: salesCompaniesWithMeetingAndFollowup,
          followups_created: salesFollowupsCreated,
          followups_completed: salesFollowupsCompleted,
          followup_completion_rate: followupExecution,
        },
        sales_execution_risk_heatmap: {
          reps: repVisualRows.map((r) => ({
            rep_id: r.rep_id,
            rep_name: r.rep_name,
            risks: {
              low_hold_rate: 'unavailable',
              low_followup_completion: 'unavailable',
              low_target_engagement: r.sales_activities > 0 && r.companies_engaged === 0 ? 'risk' : 'watch',
              low_pipeline_per_activity: (avgPipeline != null && r.sales_activities > 0 && (r.pipeline_influence / r.sales_activities) < (avgPipeline / Math.max(avgActivity ?? 1,1))*0.5) ? 'risk' : 'healthy',
              low_activity: (avgActivity != null && r.sales_activities < avgActivity * 0.5) ? 'risk' : (avgActivity != null && r.sales_activities < avgActivity * 0.85 ? 'watch' : 'healthy'),
            },
          })),
        },
        pipeline_per_activity_gauge: {
          pipeline_per_sales_activity: teamPPSA,
          expected_pipeline_per_sales_activity: expectedPipelinePerSalesActivity,
          score: (teamPPSA != null && expectedPipelinePerSalesActivity > 0) ? Math.min((teamPPSA / expectedPipelinePerSalesActivity) * 100, 100) : null,
          interpretation: (teamPPSA != null && expectedPipelinePerSalesActivity > 0) ? `${Math.round((teamPPSA / expectedPipelinePerSalesActivity) * 100)}% of target` : null,
        },
      },
      sales_execution: {
