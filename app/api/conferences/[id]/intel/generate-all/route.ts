...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const companyIds = targetCompanyRows.rows.map(r => Number(r.company_id));
  ...
  const repRows = await db.execute({
    sql: `SELECT ca.company_id, u.display_name FROM company_assignments ca JOIN users u ON u.id = ca.user_id WHERE ca.company_id IN (${companyIds.map(() => '?').join(',')})`,
    args: [...companyIds],
  });
  ...
  // Deduplicate by company_id (take first tier seen, which is highest priority due to ORDER BY)
  const seen = new Set<number>();
  const companies: Array<{
    company_id: number; company_name: string; company_type: string | null;
    industry: string | null; wse: number | null; tier: string;
  }> = [];
  ...
  await db.execute({
    sql: `UPDATE conferences SET intel_refresh_count = intel_refresh_count + 1, intel_last_refresh_at = datetime('now') WHERE id = ?`,
    args: [conferenceId],
  });
        ...
        await Promise.all(batch.map(async company => {
          try {
            const result = await generateCompanyIntel({
              ...
              repNames: repsByCompany.get(company.company_id) ?? [],
              icpPainPoints,
              icpTriggerEvents,
