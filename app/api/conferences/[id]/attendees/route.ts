...
// decouple_only: when true, only removes the conference link — does NOT delete orphaned attendees/companies
export async function DELETE(
  ...
  try {
    ...
    if (!decouple_only) {
      ...
      // Decouple only — just remove the conference link, keep all records
      await db.execute({
        sql: `DELETE FROM conference_attendees WHERE conference_id = ? AND attendee_id IN (${placeholders})`,
        args: [params.id, ...attendee_ids],
      });
