...
export async function GET(request: NextRequest) {
  ...
  const conferences = await Promise.all((res.rows as Row[]).map(async (r) => {
    ...
    // --- Component 2: Target Opportunity ---
    // Tiers stored as '1' (Must Target), '2' (High Priority), '3' (Worth Engaging), 'unassigned' (Monitor)
    ...
    const toRow = targetRows[0];
    const totalTargets = Number(toRow?.total_targets ?? 0);
    const mustCount = Number(toRow?.must_target_count ?? 0);
    const highCount = Number(toRow?.high_priority_count ?? 0);
    const worthCount = Number(toRow?.worth_engaging_count ?? 0);
    const weightedLikelihood = totalTargets > 0
      ? (mustCount * mustConv + highCount * highConv + worthCount * worthConv) / totalTargets
      : 0;
    const targetOpportunityScore: number | null = toRow != null && totalTargets > 0
      ? Math.min(Math.round((weightedLikelihood / 0.15) * 100), 100)
      : null;
    ...
    // --- Component 6: Strategic Value (not yet implemented) ---
    const strategicValueScore: number | null = null;

    // --- Assemble final score using confidence multiplier approach ---
