...
// Helper functions shared with targeting route
...
async function runTargetingForConference(
  ...
  try {
    ...
    console.error(`[calendar-intelligence] runTargetingForConference error for conference ${conferenceId}:`, err);
    return null;
  }
}
...
function computeTargetOpportunityScore(agg: TargetingAggregation): number {
  ...
  return Math.min(Math.round(
    ...
    highScore * 0.30 +
    worthScore * 0.15 +
    avgTargetPriorityScore * 0.25 +
    actionableRateScore * 0.10,
  ), 100);
}
...
function determineRecommendationTier(
  ...
  // Low-confidence gate: fewer than 4 of 6 components available caps recommendation
...
export async function GET(request: NextRequest) {
  ...
  // Load global config and conference data in parallel
  const [settingsRes, seniorityRes, functionRes, actionsRes, prospectTypeRes, effectivenessRes, res] = await Promise.all([
  ...
  // Build label maps for targeting engine
  ...
  // Prospect type for targeting engine
  ...
  // Build targeting scoring config (used for all conferences)
  ...
  // Conversion rates for Commercial Potential
  ...
  const conferences = await Promise.all((res.rows as Row[]).map(async (r) => {
    ...
    // --- Component 2: Target Opportunity — sourced from targeting engine ---
    ...
    // --- Component 6: Strategic Value — sourced from avg relationship leverage score ---
