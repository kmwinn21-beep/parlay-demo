...
export function ConferenceOfflineSync({ conferenceId }: { conferenceId: number }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      // Animate progress to ~80% while fetching
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1.5 py-1 px-2 text-sm font-medium text-brand-primary whitespace-nowrap">
          ...
          <div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <div
              className="h-1.5 bg-brand-primary rounded-full transition-all duration-200"
