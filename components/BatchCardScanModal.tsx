...
function RightCard({ card, onConfirm, onDismissMatch, onShowAddForm, onAddFormChange, onAddFormCompanySelect, onInitiateAdd, saving }: RightCardProps) {
...
export function BatchCardScanModal({ conferenceId, initialCards, onClose, onDone, onConfirmed }: Props) {
  ...
  /**
   * Confirm a match, or add the drafted attendee, and save immediately. There
   * is no interstitial prompt: notes and status are collected by the interaction
   * screen that onConfirmed opens, so asking here only duplicated it.
   */
  const saveCard = useCallback(async (localId: string, type: 'confirm' | 'add', matchAttendeeId?: number, matchCompanyId?: number | null) => {
    ...
    try {
      ...
      updateCard(localId, { status: type === 'confirm' ? 'confirmed' : 'added' });
