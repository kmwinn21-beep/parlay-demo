...
function FollowUpStatusBadge({ status }: { status: RepPerformanceRow['companies'][number]['followUpStatus'] }) {
  ...
  return (
    <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-medium border ${map[status]}`}>
      {labels[status]}
    </span>
  );
}
...
function HealthChip({ delta }: { delta: number }) {
  ...
  const up = delta > 0;
  return (
    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${up ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
      {up ? `+${delta}` : delta}
    </span>
  );
}
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between gap-3">
        ...
        <span className="text-[10px] text-gray-400">{rep.contactsCaptured} contacts</span>
      </div>
      ...
      <div className="px-4 py-2 border-b border-gray-100">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] text-gray-500 font-medium">Follow-up Rate</span>
          <span className="text-[10px] text-gray-400">{rep.followUpsCompleted}/{rep.followUpsCreated}</span>
        </div>
      ...
      {rep.companies.length > 0 && (
        <div className="px-4 py-3">
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Companies</p>
          <div className="space-y-1.5">
            {rep.companies.slice(0, 6).map((c, i) => (
              <div key={i} className="flex items-center gap-2 min-w-0">
                <Link href={`/companies/${c.company_id}`} className="text-[11px] text-brand-primary hover:text-brand-secondary truncate flex-1 min-w-0">
                  {c.company_name}
                </Link>
                <div className="flex items-center gap-1 flex-shrink-0">
                  {c.icp === 'Yes' && <span className="badge-green text-[9px] px-1.5 py-0">ICP</span>}
            ...
            {rep.companies.length > 6 && (
              <p className="text-[10px] text-gray-400">+{rep.companies.length - 6} more</p>
            )}
          </div>
        </div>
      )}
...
export function RepPerformanceTab({ repPerformance }: { repPerformance: RepPerformance }) {
  ...
  const sorted = [...repPerformance].sort((a, b) => b.contactsCaptured - a.contactsCaptured);
  const maxContacts = Math.max(1, ...sorted.map(r => r.contactsCaptured));

  return (
    <div className="space-y-6">
      {/* Leaderboard bar chart */}
      <div className="rounded-xl border border-gray-200 p-4 bg-white space-y-3">
        <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Contacts Captured by Rep</h4>
        {sorted.map(rep => (
          <div key={rep.repName} className="flex items-center gap-3">
            <span className="text-xs text-gray-700 font-medium w-28 truncate flex-shrink-0">{rep.repName}</span>
            <div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden">
              <div className="h-3 rounded-full" style={{ width: `${Math.round((rep.contactsCaptured / maxContacts) * 100)}%`, backgroundColor: '#223A5E' }} />
            </div>
            <span className="text-xs text-gray-600 w-8 text-right flex-shrink-0">{rep.contactsCaptured}</span>
          </div>
        ))}
      </div>

      {/* Follow-up rate comparison */}
      <div className="rounded-xl border border-gray-200 p-4 bg-white space-y-3">
        <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Follow-up Completion by Rep</h4>
        {sorted.map(rep => (
          <div key={rep.repName} className="flex items-center gap-3">
            <span className="text-xs text-gray-700 font-medium w-28 truncate flex-shrink-0">{rep.repName}</span>
            <FuBar rate={rep.followUpRate} />
