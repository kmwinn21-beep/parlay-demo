...
const LENS_PRESETS: Record<string, { label: string; weights: LensWeights }> = {
  ...
  pipeline_focused:   { label: 'Pipeline Focused',   weights: { audienceFit: 20, targetOpportunity: 30, engagementCapture: 10, commercialPotential: 25, costJustification: 10, strategicValue: 5 } },
  relationship_heavy: { label: 'Relationship Heavy', weights: { audienceFit: 20, targetOpportunity: 15, engagementCapture: 25, commercialPotential: 15, costJustification: 10, strategicValue: 15 } },
  cost_conscious:     { label: 'Cost Conscious',     weights: { audienceFit: 20, targetOpportunity: 20, engagementCapture: 10, commercialPotential: 15, costJustification: 30, strategicValue: 5 } },
};
...
export function StrategicLens({ score, conferenceId: _conferenceId }: Props) {
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      {/* Adjusted score */}
      <div className={`rounded-lg p-3 border ${totalValid ? 'bg-blue-50 border-blue-200' : 'bg-red-50 border-red-200'}`}>
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold text-gray-800">Lens Score</p>
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-gray-900">{lensScore ?? '—'}</span>
            {delta != null && delta !== 0 && (
        ...
        {!totalValid && (
          <p className="text-xs text-red-600 mt-1">Weights must sum to 100 (current: {Math.round(total)})</p>
        )}
      </div>
