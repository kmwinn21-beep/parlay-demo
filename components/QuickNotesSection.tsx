...
function AddNoteModal({ onClose, onSave }: { onClose: () => void; onSave: (content: string) => Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 sm:p-4" onClick={onClose}>
      ...
      <div className="modal-sheet-mobile bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md p-6 max-h-[92vh] overflow-y-auto flex flex-col" onClick={e => e.stopPropagation()}>
...
// ── Note Card ─────────────────────────────────────────────────────────────────
// ── Conference tag pill — click to attribute a floor note to a conference ──────
function NoteCard({ note, conferences, onDelete, onAssign, onEdit, onConferenceUpdated }: {
  note: QuickNote; conferences: Conference[]; onDelete: (id: number) => void; onAssign: () => void;
  onEdit: (id: number, content: string) => Promise<void>; onConferenceUpdated: (note: QuickNote) => void;
}) {
  ...
  // search input (rather than a small dropdown anchored to the pill) so
  // there's room to actually type and see results; the results list is
  // portaled to <body> so it isn't clipped by the pill row's horizontal
  // scroll container or the notes list's vertical scroll container.
  ...
  const filteredConferences = conferences.filter(c => c.name.toLowerCase().includes(confSearch.toLowerCase()));

  const updatePickerPos = useCallback(() => {
    if (!pickerRowRef.current) return;
    const rect = pickerRowRef.current.getBoundingClientRect();
  ...
  useEffect(() => {
    if (!pickerOpen) return;
    const h = (e: MouseEvent) => {
      if (pickerRowRef.current?.contains(e.target as Node)) return;
      if (pickerListRef.current?.contains(e.target as Node)) return;
      setPickerOpen(false);
    };
    document.addEventListener('mousedown', h);
    return () => document.removeEventListener('mousedown', h);
  ...
  const pickConference = async (conf: Conference | null) => {
    setPickerOpen(false);
    setConfSaving(true);
    try {
      const res = await fetch(`/api/quick-notes/${note.id}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conference_id: conf?.id ?? null }),
      });
      if (!res.ok) throw new Error();
      onConferenceUpdated(await res.json() as QuickNote);
    } catch {
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 bg-white hover:border-gray-300 transition-colors">
      <div className="flex items-center justify-between gap-2 mb-2">
        {pickerOpen ? (
          <div ref={pickerRowRef} className="flex items-center gap-1.5 flex-1 min-w-0">
            <input
              autoFocus
              value={confSearch}
              ...
              className="flex-1 min-w-0 px-3 py-1.5 text-sm border border-brand-secondary/40 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-secondary"
            />
            <button type="button" onClick={() => setPickerOpen(false)}
              className="flex-shrink-0 text-gray-400 hover:text-gray-600 transition-colors p-1" aria-label="Cancel">
          ...
          <>
            <div className="flex items-center gap-1 min-w-0 flex-1">
              <button type="button" onClick={() => scrollPills(-1)}
                className="hidden sm:block flex-shrink-0 text-gray-300 hover:text-gray-600 transition-colors" aria-label="Scroll left">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
              </button>
              <div ref={pillsRef} className="flex items-center gap-1.5 min-w-0 overflow-x-auto scrollbar-hide flex-nowrap">
                {note.tag === 'card-badge' && (
                ...
                <button
                  type="button" disabled={confSaving}
              ...
              <button type="button" onClick={() => scrollPills(1)}
                className="hidden sm:block flex-shrink-0 text-gray-300 hover:text-gray-600 transition-colors" aria-label="Scroll right">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <div className="relative" ref={menuRef}>
                ...
                {menuOpen && (
                  <div className="absolute top-full right-0 mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1.5 z-20">
                    <p className="px-3 pb-1.5 mb-1 border-b border-gray-100 text-xs text-gray-400 truncate">{fullTimestamp}</p>
      ...
      {pickerOpen && pickerPos && typeof document !== 'undefined' && createPortal(
        ...
        >
          {note.conference_id != null && (
            <button type="button" onClick={() => pickConference(null)}
              className="w-full text-left px-3 py-2 text-sm text-red-500 hover:bg-red-50 transition-colors border-b border-gray-50">
              Remove event
            </button>
          )}
          {filteredConferences.length === 0 ? (
            <p className="text-sm text-gray-400 px-3 py-2">No conferences found</p>
          ) : filteredConferences.map(c => (
            <button key={c.id} type="button" onClick={() => pickConference(c)}
              className={`w-full text-left px-3 py-2 text-sm hover:bg-blue-50 hover:text-brand-secondary transition-colors ${note.conference_id === c.id ? 'bg-blue-50 text-brand-secondary font-medium' : 'text-gray-700'}`}>
              {c.name}
            </button>
          ))}
        </div>,
...
export function QuickNotesSection({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/conferences?nav=1').then(r => r.ok ? r.json() : [])
      .then((data: unknown) => setConferences(Array.isArray(data) ? data.map((c: { id: number; name: string }) => ({ id: c.id, name: c.name })) : []))
      .catch(() => {});
  }, []);

  const handleConferenceUpdated = useCallback((updated: QuickNote) => {
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => { if (cameraMenuRef.current && !cameraMenuRef.current.contains(e.target as Node)) { setShowCameraMenu(false); setBadgeSourceStep(false); } };
    document.addEventListener('mousedown', h);
    return () => document.removeEventListener('mousedown', h);
  }, [showCameraMenu]);
  ...
  const handleSaveNote = useCallback(async (content: string, tag?: string | null): Promise<QuickNote | null> => {
    const res = await fetch('/api/quick-notes', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content, tag: tag ?? null, conference_id: activeConference?.id ?? null }),
    });
    if (res.ok) {
  ...
  const handleScanAssignLater = useCallback(async (card: BadgeScanCard, secondaryTag?: string) => {
    ...
    const res = await fetch('/api/quick-notes', {
      ...
      body: JSON.stringify({
        ...
        conference_id: activeConference?.id ?? null,
      }),
    });
    if (res.ok) {
      const note = await res.json() as QuickNote;
  ...
  return (
    <div className={`card h-full flex flex-col overflow-hidden ${className}`}>
      <div className="flex items-center justify-between mb-1 flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5 lg:hidden">
          <div className="relative" ref={cameraMenuRef}>
            ...
            {showCameraMenu && (
              <div className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50 min-w-[190px]">
                {badgeSourceStep ? (
                  <>
                    <button type="button" onClick={() => setBadgeSourceStep(false)}
                      ...
                      <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                      Badge / Card
                    </button>
                    <button type="button" onClick={() => { setShowCameraMenu(false); setBadgeSourceStep(false); badgeFileRef.current?.click(); }}
                    ...
                    <button type="button" onClick={() => { setShowCameraMenu(false); setBadgeSourceStep(false); badgeLibraryRef.current?.click(); }}
                      className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-brand-secondary transition-colors">
                  ...
                  <>
                    <button type="button" onClick={() => setBadgeSourceStep(true)}
                      className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-brand-secondary transition-colors">
                      <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2" /></svg>
                      <span className="flex-1 text-left">Scan Badge/Card</span>
                      <svg className="w-3.5 h-3.5 flex-shrink-0 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </button>
      ...
      <input ref={badgeLibraryRef} type="file" accept="image/*" className="hidden"
        onChange={e => { const f = e.target.files?.[0]; if (f) handleBadgeFile(f); e.target.value = ''; }} />
              ...
              <div ref={scrollRef} onScroll={checkScroll} className="h-full overflow-y-auto space-y-3 pr-1 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' } as React.CSSProperties}>
                {notes.map(note => (
                  <NoteCard
                    ...
                    onAssign={() => handleAssignNote(note)}
                    onEdit={handleEdit}
...
export function QuickNoteInlineModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch('/api/quick-notes', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: text.trim(), conference_id: activeConference?.id ?? null }),
    });
    setSaving(false);
    if (res.ok) {
      const note = await res.json() as QuickNote;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/50 sm:p-4" onClick={onClose}>
      <div className="modal-sheet-mobile bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md p-5 max-h-[92vh] overflow-y-auto flex flex-col" onClick={e => e.stopPropagation()}>
