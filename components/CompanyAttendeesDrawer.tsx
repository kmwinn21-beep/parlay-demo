...
export function CompanyAttendeesDrawer({ companyId, companyName, conferenceLabel, conferenceId, attendees, zClass = 'z-50', onClose }: Props) {
  ...
  const content = (
    <div className={`fixed inset-0 ${zClass}`}>
      ...
      {/* Attendee drawer — slides up from the bottom on mobile, in from the right on
          desktop. Opening the timeline slides it across to the far left edge, clear of
          the timeline and of the sidebar it covers on the way. Both states are anchored
          by `right` rather than one by `left` and the other by `right`, so the move is a
          single animatable value instead of a jump between two anchors. Hidden on mobile
          while the timeline is open (no room for both side by side). */}
      <div
        className={`drawer-mobile-responsive fixed inset-x-0 bottom-0 sm:inset-y-0 sm:inset-x-auto h-[85vh] sm:h-auto w-full sm:w-[480px] bg-white shadow-2xl overflow-hidden rounded-t-2xl sm:rounded-t-none sm:transition-[right] sm:duration-300 sm:ease-out ${timelineOpen ? 'hidden sm:flex sm:rounded-tr-2xl' : 'flex sm:rounded-tl-2xl'} flex-col`}
        // Percentages on a fixed element resolve against the viewport, so this
        // puts the right edge 480px in — the drawer's left edge on 0 — without
        // depending on 100vw, which counts a scrollbar the viewport doesn't have.
        style={{ right: timelineOpen ? 'calc(100% - 480px)' : 0 }}
      >
        <div
