...
import { getPersonInitials } from '@/lib/useUserOptions';
...
export interface PinnedNote {
  ...
  entity_id: number;
  pinned_by: string;
  /** `pinned_by` resolved to a rep profile, when it matched one. */
    ...
    <div className="card border-l-4 border-l-brand-highlight">
      ...
      {!sectionCollapsed && <div className="space-y-3 mt-4">
        {pinnedNotes.map(pin => {
          ...
          // pinned_by holds an email; the reader resolves it to the rep
          // profile so this pill matches the one on the note card.
          const pinnedByName = pin.pinned_by_name || pin.pinned_by;
          const initials = getPersonInitials(pinnedByName);
            ...
            <div key={pin.id} className="rounded-xl border border-gray-200 bg-white overflow-hidden">
              ...
              <div className="px-4 py-3 flex items-center justify-between gap-2 bg-gray-50 border-b border-gray-100">
                <div className="flex flex-wrap items-center gap-2 min-w-0">
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-brand-primary text-white text-xs font-medium" title={`Pinned by ${pinnedByName}`}>
