...
export function SlideInPanel({
  ...
  /** Below sm, these ride the header instead of the footer — a bottom sheet
   *  puts its footer a long way from the content it acts on. Replaces the
   *  footer at that size rather than duplicating it. */
        ...
        <div className="flex items-start justify-between gap-2 px-3 py-2.5 border-b border-gray-100 flex-shrink-0">
          <div className="min-w-0 flex-1">
            ...
            {!isDesktop && mobileHeaderActions && (
              <div className="flex items-center justify-end gap-2 mt-2">{mobileHeaderActions}</div>
            )}
          </div>
          <button
            type="button"
