...
function NoteRow({
  ...
  const notesBase = `/api/conferences/${conferenceId}/outreach/${companyId}/notes/${note.id}`;

  const toggleComments = () => {
    setCommentsOpen(v => !v);
    if (comments === null && !loadingComments) {
      setLoadingComments(true);
      fetch(`${notesBase}/comments`)
  ...
  const handlePostComment = async () => {
    ...
    try {
      const res = await fetch(`${notesBase}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ body, taggedUsers: commentTaggedIds.length > 0 ? commentTaggedIds.join(',') : null }),
      });
      if (!res.ok) throw new Error();
      const created = await res.json() as NoteComment;
  ...
  const handleDeleteComment = async (commentId: number) => {
    ...
    try {
      const res = await fetch(`${notesBase}/comments/${commentId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error();
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(notesBase, { method: 'DELETE' });
      if (!res.ok) throw new Error();
      onDeleted(note.id);
    } catch {
      ...
      <div className="min-w-0 flex-1">
        ...
        {note.activityType && (
          <p className="text-[11px] text-gray-500 mt-0.5 flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full flex-shrink-0 ${DOT_COLOR[note.activityType]}`} />
            <span className="font-medium">{ACTIVITY_LABEL[note.activityType]}</span>
          </p>
        )}
        {note.attendeeName && (
