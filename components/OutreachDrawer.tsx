...
import { useUserOptions, type UserOption } from '@/lib/useUserOptions';
import { MentionTextarea } from './MentionTextarea';
import { ThreadNoteRow, type ThreadNoteHandlers } from './ThreadNoteRow';
...
/** Adapts the shared threaded-note row onto the outreach notes endpoints. */
function NoteRow({
  note,
  conferenceId,
  companyId,
  ...
  onUpdated: (note: ThreadNote) => void;
  onDeleted: (noteId: number) => void;
}) {
  const base = `/api/conferences/${conferenceId}/outreach/${companyId}/notes/${note.id}`;

  const handlers: ThreadNoteHandlers = {
    updateNote: async (_id, body) => {
    ...
    loadComments: async () => {
      ...
      if (!res.ok) throw new Error();
      const data = await res.json() as { comments: NoteComment[] };
      return data.comments.map(c => ({
        id: c.id, body: c.body, userName: c.userName, createdAt: c.createdAt, isMine: c.isMine,
      }));
    },
    postComment: async (_id, body, taggedUserIds) => {
      const res = await fetch(`${base}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) throw new Error();
      const c = await res.json() as NoteComment;
      return { id: c.id, body: c.body, userName: c.userName, createdAt: c.createdAt, isMine: c.isMine };
    },
  ...
  return (
    <ThreadNoteRow
      note={{
        ...
        userInitials: note.userInitials,
        createdAt: note.createdAt,
        isMine: note.isMine,
        commentCount: note.commentCount,
        activityDotClass: note.activityType ? DOT_COLOR[note.activityType] : null,
        activityLabel: note.activityType ? ACTIVITY_LABEL[note.activityType] : null,
        attendeeName: note.attendeeName,
      }}
      userOptions={userOptions}
      handlers={handlers}
      onUpdated={updated => onUpdated({ ...note, body: updated.body })}
      onDeleted={onDeleted}
    />
  );
}
